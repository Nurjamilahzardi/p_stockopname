...
class BarangController extends Controller
{
    ...
    public function barangunit(Request $request)
    {
        $user = Auth::user();
        $unit = $user->unit;
        $role = $user->role;
        ...
        if($role === 'admin'){
            ...
            if ($search = $request->input('search')) {
                $query->where('barang.barang_nama', 'like', '%' . $search . '%');
            }
            ...
            return view('barangunit', ['Details' => $Details]);
        }
        else if ($role === 'user'){
            $query = DB::table('detail_transaksi')
                ->join('transaksi', 'detail_transaksi.trans_id', '=', 'transaksi.id')
            ...
            return view('user.barangunit', ['Details' => $Details]);
        }
        else {
            return abort(403, 'Anda tidak memiliki izin untuk mengakses halaman ini.');
        }
