...
class TransaksiController extends Controller
{
    ...
    public function simpanDataMasuk(Request $request)
    {
        ...
        try {
            ...
            return redirect('/transaksi')->with('success', 'Transaksi Barang Masuk berhasil disimpan');
    ...
    public function keluar() : View
    {
        ...
        $role = $user->role;
        $unit = $user->unit;
        $Users = User::when($unit != '13000', function ($query) use ($unit) {
            $allowedUnits = ['UP3 Padang', 'UP3 Solok', 'UP3 Bukittinggi', 'UP3 Payakumbuh'];
            return $query->whereHas('up', function ($subquery) use ($unit, $allowedUnits) {
        ...
        if($role === 'admin'){
            $UPs = UP::all();
            $ULPs = ULP::when($unit != '13000', function ($query) use ($unit) {
                $allowedUnits = ['UP3 Padang', 'UP3 Solok', 'UP3 Bukittinggi', 'UP3 Payakumbuh'];
                return $query->whereHas('up', function ($subquery) use ($unit, $allowedUnits) {
                    $subquery->where('up_id', $unit)->orWhereIn('up_id', $allowedUnits);
                });
            ...
            return view('transaksi.keluar',compact('UPs','ULPs','Users'));
        }else if ($role === 'user'){
            $UPs = UP::all();
            $ULPs = ULP::when($unit != '13000', function ($query) use ($unit) {
                $allowedUnits = ['UP3 Padang', 'UP3 Solok', 'UP3 Bukittinggi', 'UP3 Payakumbuh'];
            ...
            return view('user.transaksi',compact('UPs','ULPs','Users'));
        } else{
            return abort(403,'Anda tidak memiliki izin untuk mengakses halaman ini.');
        }
    }
    ...
    public function simpanDataKeluar(Request $request)
    {
        ...
        if ($role === 'admin'){
            $validator = Validator::make($request->all(), [
                ...
                'slculpkeluar' => 'nullable|exists:ulp,id',
            ...
            try {
                ...
                return redirect('/transaksi')->with('success', 'Transaksi Barang Keluar berhasil disimpan');
            } catch (\Illuminate\Database\QueryException $e) {
                if ($e->errorInfo[1] == 1062) {
    ...
    public function barangmasuk(){
        ...
        if($role === 'admin'){
            $transaksis = Transaksi::where('trans_jenis','masuk')->when($unit != '13000', function ($query) use ($unit) {
                $allowedUnits = ['UP3 Padang', 'UP3 Solok', 'UP3 Bukittinggi', 'UP3 Payakumbuh'];
                return $query->whereHas('up', function ($subquery) use ($unit, $allowedUnits) {
                    $subquery->where('up_id', $unit)->orWhereIn('up_id', $allowedUnits);
                });
            ...
            return view('user.barangmasuk', compact('transaksis', 'UPs', 'ULPs','User'));
        }
    }
    public function barangkeluar(){
        ...
        if($role === 'admin'){
            $transaksis = Transaksi::where('trans_jenis','keluar')->when($unit != '13000', function ($query) use ($unit) {
                $allowedUnits = ['UP3 Padang', 'UP3 Solok', 'UP3 Bukittinggi', 'UP3 Payakumbuh'];
                return $query->whereHas('up', function ($subquery) use ($unit, $allowedUnits) {
                    $subquery->where('up_id', $unit)->orWhereIn('up_id', $allowedUnits);
                });
            ...
            return view('user.barangkeluar', compact('transaksis', 'UPs', 'ULPs','User'));
        }
    }
    public function cetakBarangKeluar($trans_id)
    {
        ...
        if($role === 'admin'){
            ...
            ->join('users','transaksi.created_by','=','users.id')
            ->join('barang', 'detail_transaksi.barang_id', '=', 'barang.id')
            ->join('up', 'transaksi.up_id', '=', 'up.id')
            ->where('transaksi.kode_trans', 'like', 'TK%')
            ->where('transaksi.id', $trans_id)
            ->first();
    ...
    public function cetakBarangMasuk($trans_id)
    {
        $trans = DetailTransaksi::select(
            'transaksi.tanggal_trans',
            'transaksi.up_id',
            'barang.barang_code',
            'barang.barang_nama',
            'detail_transaksi.barang_quantity AS jumlah',
        ...
        ->join('barang', 'detail_transaksi.barang_id', '=', 'barang.id')
        ->join('up', 'transaksi.up_id', '=', 'up.id')
        ->where('transaksi.kode_trans', 'like', 'TM%')
        ->where('transaksi.id', $trans_id)
        ->first();
        ...
        ->join('transaksi', 'detail_transaksi.trans_id', '=', 'transaksi.id')
        ->join('up','transaksi.up_id','=','up.id')
        ->where('transaksi.kode_trans', 'like', 'TM%')->where('transaksi.id', $trans_id)
        ->get();
        
        return view('transaksi.cetak-barang-masuk', compact('trans','detail'));
    }
}